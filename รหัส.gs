...
// =========================================================
// จัดการรหัสผ่าน Setting สำหรับ Google Apps Script
// =========================================================
function getAdminPassword() {
  try {
    const prop = PropertiesService.getScriptProperties().getProperty('SMT_ADMIN_PWD');
    return prop || '1234';
  } catch (e) {
    return '1234';
  }
...
function saveAdminPassword(newPwd) {
  const p = String(newPwd || '').trim();
  if (!p) throw new Error('Password cannot be empty');
  PropertiesService.getScriptProperties().setProperty('SMT_ADMIN_PWD', p);
  return true;
}